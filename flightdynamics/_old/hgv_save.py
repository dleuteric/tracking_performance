# hgv_generate_and_save.py
import numpy as np, pandas as pd, matplotlib.pyplot as plt
from mpl_toolkits.mplot3d import Axes3D  # noqa

# ── User windows ──────────────────────────────────────────────────────────
R_MIN_KM, R_MAX_KM   =  500, 5000
LAT_MIN_DEG, LAT_MAX_DEG = -20, 20
OUTFILE = 'trajectory_set.xlsx'
...
def hgv_dot(t,y,p):
    ...
    aL = q*CL*p['S']/p['m']; aD = q*CD*p['S']/p['m']
    g0 = mu/r**2
    # J2 perturbation (radial & lat components in VTC frame)
    z_sl = np.sin(la); z_sl2 = z_sl*z_sl
    aJ2_r  = -1.5*J2*mu*Re**2/r**4*(1-3*z_sl2)
    aJ2_lat= -3*J2*mu*Re**2/r**4*z_sl*np.cos(la)
    rdot = v*np.sin(th)
    vdot = -aD - g0*np.sin(th) + aJ2_r*np.sin(th) \
           + omega_e**2*r*np.cos(la)*(np.sin(th)*np.cos(la)-np.cos(th)*np.sin(si)*z_sl)
    thdot = (aL*np.cos(b)/v) + (v/r - g0/v)*np.cos(th) + aJ2_lat*np.cos(th)/v + \
            2*omega_e*np.cos(si)*np.cos(la) + \
            (omega_e**2*r/v)*np.cos(la)*(np.cos(th)*np.cos(la)+np.sin(th)*np.sin(si)*z_sl)
    ...
    sigdot = (aL*np.sin(b)/(v*np.cos(th)+1e-9)) + (v/r)*np.cos(th)*np.sin(si)*np.tan(la) - \
             2*omega_e*(np.tan(th)*np.cos(la)*np.sin(si)-z_sl) + \
             (omega_e**2*r/(v*np.cos(th)+1e-9))*np.sin(si)*np.cos(la)*z_sl
...
# ── helpers: down‑range & ECEF conversion ─────────────────────────────────
...
# ── Generate trajectories with dual filter ───────────────────────────────
accepted=[]; max_tries=4000
for label,a_fn,b_fn in man:
    for tries in range(1,max_tries+1):
        ...
        if R_MIN_KM<=R<=R_MAX_KM:
            accepted.append((label,T,Y,R,lat0_deg))
            print(f"{label}: φ0={lat0_deg:.1f}°, R={R:.0f} km (tries {tries})")
            break
    else:
        print(f"{label}: not found in {max_tries} draws")

# ── Save to Excel ─────────────────────────────────────────────────────────
with pd.ExcelWriter(OUTFILE) as writer:
    for idx,(lab,T,Y,R,lat0) in enumerate(accepted,1):
        r,lo,la=Y[:,0],Y[:,1],Y[:,2]; x,y,z=ecef(r,lo,la)
        dt=np.diff(T,prepend=T[0])
        vx=np.gradient(x,T); vy=np.gradient(y,T); vz=np.gradient(z,T)
        ax=np.gradient(vx,T); ay=np.gradient(vy,T); az=np.gradient(vz,T)
        df=pd.DataFrame({'t_s':T,'x_m':x,'y_m':y,'z_m':z,
                         'vx_mps':vx,'vy_mps':vy,'vz_mps':vz,
                         'ax_mps2':ax,'ay_mps2':ay,'az_mps2':az})
        df.to_excel(writer,sheet_name=f'Traj{idx}_{lab}',index=False)
...
ax3d.set_title('3‑D Trajectories (J₂ + CSV logging)'); ax3d.legend(bbox_to_anchor=(1.04,1))
