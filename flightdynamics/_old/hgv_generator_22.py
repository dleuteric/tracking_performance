...
# ======== USER GATES ========
NUM_TRAJ = 3  # how many trajectories to output
R_MIN_KM, R_MAX_KM = 500, 6000  # down-range window
LAT_MIN_DEG, LAT_MAX_DEG = 50, 70  # launch-latitude window
LON_MIN_DEG, LON_MAX_DEG = -1, 1  # launch-longitude window
OUTFILE = 'trajectory_set2.xlsx'
PROP_TIME = 2000.0  # seconds, propagation time for RK
...
def hgv_dot(t, y, p):
    ...
    g0 = mu / r ** 2
    zsl = np.sin(la);
    aJ2_r = -1.5 * J2 * mu * Re ** 2 / r ** 4 * (1 - 3 * zsl ** 2)
...
np.random.seed(0)
tries_global = 0
success_times = []  # Track time for successful trajectories
pbar = tqdm(total=NUM_TRAJ, desc="Generating trajectories", unit="traj")
start_time = time.time()

while len(accepted) < NUM_TRAJ:
    tries_global += 1
    lab, a_fn, b_fn = man[np.random.randint(0, len(man))]  # Pick random manoeuvre class
    ...
    sig0 = np.deg2rad(np.random.uniform(-60, 60))
    y0 = np.array([Re + h0, np.deg2rad(lon0_deg), np.deg2rad(lat0_deg), v0, th0, sig0])

    # Debug print: Initial conditions
    print(f"\nTry {tries_global}: {lab}, h0={h0 / 1e3:.1f} km, v0={v0 / 1e3:.2f} km/s, "
          f"lat0={lat0_deg:.1f}°, lon0={lon0_deg:.1f}°, th0={np.rad2deg(th0):.2f}°, "
          f"sig0={np.rad2deg(sig0):.2f}°")

    traj_start = time.time()
    T, Y = propagate(y0, p=dict(m=m_nom, S=S_nom, alpha=a_fn, bank=b_fn))

    if Y[-1, 0] <= Re + 1.0:
        print(f"Rejected: Trajectory hit surface (r={Y[-1, 0] / 1e3:.1f} km)")
        continue

    R = downrange(np.deg2rad(lon0_deg), np.deg2rad(lat0_deg), Y[-1, 1], Y[-1, 2])
    if not (R_MIN_KM <= R <= R_MAX_KM):
        print(f"Rejected: Downrange {R:.0f} km outside [{R_MIN_KM}, {R_MAX_KM}] km")
        continue

    # Successful trajectory
    traj_time = time.time() - traj_start
    success_times.append(traj_time)
    accepted.append((lab, T, Y, R, lat0_deg, lon0_deg))
    print(f"Accepted [{len(accepted)}/{NUM_TRAJ}]: {lab}, R={R:.0f} km, "
          f"time={traj_time:.2f} s")
    ...
    if success_times:
        ...
        remaining_traj = NUM_TRAJ - len(accepted)
        eta = remaining_traj * avg_time
        pbar.set_postfix({"ETA": f"{eta:.0f} s"})

pbar.close()
total_time = time.time() - start_time
print(f"\nCompleted {NUM_TRAJ} trajectories in {total_time:.0f} s "
      f"(avg {total_time / NUM_TRAJ:.1f} s/traj, {tries_global} total tries)")

# ── EXCEL LOGGING ----------------------------------------------------------
with pd.ExcelWriter(OUTFILE) as xl:
    for n, (lab, T, Y, R, lat0, lon0) in enumerate(accepted, 1):
...
for i, (lab, T, Y, R, lat0, lon0) in enumerate(accepted):
    h = Y[:, 0] - Re;
    lon = np.rad2deg(Y[:, 1]);
    lat = np.rad2deg(Y[:, 2])
    lbl = f"{lab} (R={R:.0f} km, φ0={lat0:.0f}°, λ0={lon0:.0f}°)"
...
ax3d.set_title('3‑D Trajectories (J₂ + Earth‑guard)');
ax3d.legend(bbox_to_anchor=(1.04, 1))
ax2d.set_xlabel('Lon (°)');
ax2d.set_ylabel('Lat (°)');
ax2d.set_title('2‑D Ground‑tracks')
ax2d.legend(bbox_to_anchor=(1.04, 1));
ax2d.grid(True);
plt.tight_layout()

# EXTRA 3‑D VIEW WITH EARTH SPHERE -----------------------------------------
fig2 = plt.figure(figsize=(6, 6));
axE = fig2.add_subplot(111, projection='3d')
...
for i, (lab, _, Y, _, _, _) in enumerate(accepted):
    x, y, z = ecef(Y[:, 0], Y[:, 1], Y[:, 2])
    axE.plot(x, y, z, color=colors[i], lw=.8, ls='--')
axE.set_title('Trajectory clearance vs Earth')
axE.set_box_aspect([1, 1, 1])
axE.set_xlim(-Re * 1.2, Re * 1.2)
axE.set_ylim(-Re * 1.2, Re * 1.2)
axE.set_zlim(-Re * 1.2, Re * 1.2)
plt.tight_layout()
plt.show()