...
I/O contract:
  ...
  * Output CSV: t_s, time, x_km, y_km, z_km, vx_kms, vy_kms, vz_kms, frame, ewrls_order, ewrls_theta
...
def _ew_rls_1d(t_rel: np.ndarray,
    ...
    if t_scale is None:
        span = float(max(1.0, (float(t_rel.max()) - float(t_rel.min()))))
    ...
    for k in range(N):
        u = _poly_regressor(float(t_s[k]), d)         # (d,)
        denom = float(theta + u @ (P @ u))            # scalar
        K = (P @ u) / denom                           # (d,)
        innov = float(z[k] - u @ a)
        a = a + K * innov
        P = (P - np.outer(K, u) @ P) / theta          # EW covariance update
...
def run_ewrls_on_csv(in_csv: str,
    ...
    # ISO8601 time strings for plotting compatibility
