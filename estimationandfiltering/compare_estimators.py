...
def read_estimator_csv(csv_path: Path) -> pd.DataFrame:
    ...
    # Column mapping (km + seconds expected but robust to variants)
    def pick(df, cands):
        for c in cands:
            if c in df:
                return c
        for c in df.columns:
            n = c.lower()
            if any(cc in n for cc in cands):
                return c
        raise KeyError(f"Missing columns {cands} in {csv_path.name}")

    tcol = pick(df, ["t_s", "time_s", "t", "epoch_s", "time", "time_iso"])
    ...
    def is_m(col):
        s = str(col)
        return s.endswith("_m") or s.endswith("_icrf_m") or s.endswith("_ecef_m")
...
def rms_between(kf: pd.DataFrame, ew: pd.DataFrame) -> float:
    ...
    def interp_col(df, col):
        return np.interp(t, df["t_s"].to_numpy(float), df[col].to_numpy(float))
...
def plot_errors_overlay(tgt: str, ekf: Optional[Dict[str, np.ndarray]], eew: Optional[Dict[str, np.ndarray]], out_dir: Path):
    ...
    def rel_t(err: Optional[Dict[str, np.ndarray]]):
        if err is None:
            return None
        t = err["t"]
        return t - t[0]
...
def compare_run(run_id: str, out_dir: Path, cfg: Optional[str] = None) -> pd.DataFrame:
    ...
    for tgt in targets:
        ...
        row = {"target": tgt}
        if ekf:
            row.update({f"KF_{k}": v for k, v in metrics_from_errors(ekf).items()})
        if eew:
            row.update({f"EW_{k}": v for k, v in metrics_from_errors(eew).items()})
...
if __name__ == "__main__":
    ...
    df = compare_run(args.run_id, out_dir=out_dir, cfg=args.config)
    if df.empty:
